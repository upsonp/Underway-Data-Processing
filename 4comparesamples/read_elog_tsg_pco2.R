...
#' 
#' @param ELOG TSG logs (pathrawdata) directory, TSG processed files directory (pathprocessed), and bottle data directory (pathbottle)
#' 
#' @output is in the \4comparesamples folder and includes:
# - Bottle_TSG_PCO2_QAT.csv - table: TSG data average of a minute (12 values) Includes PCO2 data  matched with sample data 
# - TSG_PCO2_bottletable.csv - table: TSG data when the sample was taken, 12 readings per minute 
# - TSG_PCO2_samples.csv -  table: time, date, lat, lon, ID of each sample 
...
read.elog_tsg_pco2 <- function(pathrawdata,pathprocessed,pathbottle){
...
for (t in 1:length(TSG_samples$time)){
  ...
  for (h in 1:length(tsgposix$time)){
    ...
    if (ztsg == z2){
      ...
      TSG_bottletable[x,13] <- 0
      TSG_bottletable[x,14] <- 0
      x=x+1
      
    }
  }
  for (c in 1:length(pco2posix$time)){
    ...
    if (ctsg == c2){
      ...
      TSG_bottletable[b,13] <- pco2all$CO2_ppm[c]
      TSG_bottletable[b,14] <- pco2all$time_pco2[c]
      b=b+1
...
write.csv(TSG_samples, file = "4comparesamples/TSG_PCO2_samples.csv")
write.csv(TSG_bottletable, file = "4comparesamples/TSG_PCO2_bottletable.csv")
write_delim(TSG_bottletable, file = "4comparesamples/TSG_PCO2_bottletable", delim = ",")
...
TSG_samples <- data.frame(read.csv("4comparesamples/TSG_PCO2_samples.csv", stringsAsFactors = FALSE))
TSG_bottletable <- data.frame(read.csv("4comparesamples/TSG_PCO2_bottletable.csv", stringsAsFactors = FALSE))
...
write_delim(TSG_samples, file = "4comparesamples/Bottle_TSG_PCO2_QAT", delim = ",")
write.csv(TSG_samples, file = "4comparesamples/Bottle_TSG_PCO2_QAT.csv")
